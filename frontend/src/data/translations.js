// Tajik (tj) strings below are a draft machine-assisted translation and
// should be reviewed by a native speaker before any public demo.
const translations = {
  ru: {
    lang: { ru: 'РУС', tj: 'ТҶ' },
    nav: {
      home: 'Главная',
      catalog: 'Каталог',
      about: 'О нас',
      contacts: 'Контакты',
    },
    header: {
      search: 'Поиск',
      cart: 'Корзина',
      profile: 'Профиль',
    },
    hero: {
      titleLine1: 'Подберём уход',
      titleLine2: 'именно для вас',
      description:
        'Расскажите нашему AI-консультанту о своей коже, волосах или проблеме — и получите персональные рекомендации из каталога HAYAT BEAUTY.',
      cta: 'Начать диалог',
    },
    benefits: {
      heading: 'Почему выбирают нас',
      items: [
        {
          id: 'personal',
          icon: 'sparkle',
          title: 'Персональные рекомендации',
          description: 'Учитываем ваш тип кожи, волос и потребности',
        },
        {
          id: 'trusted',
          icon: 'shield',
          title: 'Только проверенные товары',
          description: 'Оригинальная продукция от лучших брендов',
        },
        {
          id: 'care',
          icon: 'heart',
          title: 'Забота о вас',
          description: 'Мы рядом на каждом этапе вашего ухода',
        },
        {
          id: 'delivery',
          icon: 'truck',
          title: 'Удобная доставка',
          description: 'Быстро по Душанбе и всей стране',
        },
      ],
    },
    categories: {
      heading: 'Популярные категории',
      viewAll: 'Смотреть все',
      items: [
        {
          id: 'face',
          title: 'Уход за лицом',
          subtitle: 'Забота о вашей коже',
          featured: true,
          image: 'https://hbshop.tj/media/items/Frame_2_-_2025-04-01T1707.jpg',
        },
        { id: 'hair', title: 'Уход за волосами', image: 'https://hbshop.tj/media/items/Frame_2_Lhnhm7i.jpg' },
        { id: 'makeup', title: 'Макияж', image: 'https://hbshop.tj/media/items/201_eWDP7ti.jpg' },
        { id: 'body', title: 'Тело', image: 'https://hbshop.tj/media/items/Frame_2_-_2025-02-22T1445.jpg' },
        { id: 'fragrance', title: 'Ароматы', image: 'https://hbshop.tj/media/items/Frame_1_3_PihwfCP.jpg' },
      ],
    },
    catalogPage: {
      heading: 'Каталог',
      subheading: 'Топ товаров в каждой категории',
      loading: 'Загружаем товары...',
      error: 'Не получилось загрузить каталог. Попробуйте обновить страницу.',
      empty: 'В этой категории пока нет товаров.',
    },
    ai: {
      brand: 'HAYAT BEAUTY',
      role: 'AI Beauty Advisor',
      greeting: 'Привет! Я AI-консультант HAYAT BEAUTY. Чем могу помочь сегодня?',
      welcomeTitle: 'HAYAT BEAUTY AI',
      welcomeSubtitle: 'Ваш персональный beauty-консультант',
      welcomeDescription: 'Помогу подобрать уход, косметику и продукты из каталога HAYAT BEAUTY.',
      quickActions: [
        { id: 'skincare', label: 'Уход за кожей', prompt: 'Подскажите уход за кожей, который подойдёт именно мне' },
        { id: 'makeup', label: 'Макияж', prompt: 'Помогите подобрать макияж' },
        { id: 'hair', label: 'Волосы', prompt: 'Нужен уход за волосами' },
        { id: 'fragrance', label: 'Ароматы', prompt: 'Посоветуйте аромат' },
      ],
      recommendationsLabel: 'Рекомендации для вас',
      placeholder: 'Расскажите, что вас интересует...',
      send: 'Отправить',
      typing: 'AI печатает...',
      addToCart: 'В корзину',
      added: 'Добавлено',
      cheaperOption: 'Дешевле?',
      cheaperUserLine: 'Это дороговато',
      cheaperFound: 'Конечно, вот более доступный вариант с похожим назначением.',
      cheaperUnavailable: 'Это уже самый доступный вариант в этой категории.',
      closeChat: 'Закрыть чат',
      closeBubble: 'Скрыть подсказку',
      error: 'Извините, что-то пошло не так. Попробуйте ещё раз.',
      purchaseMessage:
        'Ваш заказ #{orderId} успешно оформлен. Я подготовила инструкцию по использованию выбранных средств. Выберите язык инструкции:',
      languageRu: 'Русский',
      languageTj: 'Тоҷикӣ',
      pdfTitle: 'Инструкция по использованию средств HAYAT BEAUTY',
      pdfCaptionRu: 'Русский · PDF',
      pdfCaptionTj: 'Тоҷикӣ · PDF',
      pdfDownload: 'Скачать инструкцию',
      pdfPreparing: 'Готовим PDF...',
      pdfError: 'Не получилось подготовить PDF. Попробуйте ещё раз.',
      pdfRetry: 'Повторить',
    },
    cart: {
      title: 'Корзина',
      empty: 'Корзина пока пуста',
      total: 'Итого',
      currency: 'сомони',
      checkout: 'Оформить заказ',
      checkingOut: 'Оформляем...',
      remove: 'Удалить',
      checkoutReviewTitle: 'Оформление заказа',
      confirmOrder: 'Подтвердить заказ',
      confirmingOrder: 'Оформляем ваш заказ...',
      backToCart: 'Назад',
      demoDisclaimer: 'Демонстрационный заказ — оплата не производится.',
      successTitle: 'Заказ успешно оформлен',
      successText: 'Спасибо за покупку в HAYAT BEAUTY.',
      instructionsHint: 'Инструкция по продуктам появится в AI-консультанте.',
      continueShopping: 'Продолжить покупки',
      close: 'Закрыть корзину',
    },
    footer: {
      about:
        'HAYAT BEAUTY — премиальная косметика в Душанбе с персональным AI-консультантом по красоте.',
      contactsHeading: 'Контакты',
      address: 'г. Душанбе, пр. Рудаки',
      phone: '+992 000 00 00 00',
      rights: 'Все права защищены.',
    },
    auth: {
      loginTab: 'Вход',
      registerTab: 'Регистрация',
      nameLabel: 'Имя',
      namePlaceholder: 'Как вас зовут',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      passwordLabel: 'Пароль',
      passwordPlaceholder: '••••••••',
      loginSubmit: 'Войти',
      registerSubmit: 'Создать аккаунт',
      loginPending: 'Входим...',
      registerPending: 'Создаём аккаунт...',
      close: 'Закрыть',
      loggedInAs: 'Вы вошли как',
      logout: 'Выйти',
      account: 'Личный кабинет',
    },
    account: {
      title: 'Личный кабинет',
      purchaseHistory: 'История покупок',
      empty: 'Пока нет покупок',
      quantity: 'Кол-во',
      purchasedOn: 'Куплено',
      depletionLabel: 'Хватит примерно',
      loading: 'Загружаем...',
      loadError: 'Не удалось загрузить историю покупок',
      notifications: 'Уведомления',
      notificationsEmpty: 'Пока нет уведомлений',
      simulate24h: 'Симулировать: прошло 24 часа',
      cartReminder: 'Не забудьте оформить заказ — в корзине есть неоплаченные товары',
      depletionWarning: '{title} скоро закончится',
    },
  },
  tj: {
    lang: { ru: 'РУС', tj: 'ТҶ' },
    nav: {
      home: 'Асосӣ',
      catalog: 'Феҳрист',
      about: 'Дар бораи мо',
      contacts: 'Тамос',
    },
    header: {
      search: 'Ҷустуҷӯ',
      cart: 'Сабад',
      profile: 'Профил',
    },
    hero: {
      titleLine1: 'Нигоҳубини',
      titleLine2: 'махсус барои шумо',
      description:
        'Ба мушовири AI мо дар бораи пӯст, мӯй ё мушкилотатон нақл кунед — ва тавсияҳои шахсӣ аз феҳристи HAYAT BEAUTY гиред.',
      cta: 'Оғози гуфтугӯ',
    },
    benefits: {
      heading: 'Чаро моро интихоб мекунанд',
      items: [
        {
          id: 'personal',
          icon: 'sparkle',
          title: 'Тавсияҳои шахсӣ',
          description: 'Намуди пӯст, мӯй ва эҳтиёҷоти шуморо ба назар мегирем',
        },
        {
          id: 'trusted',
          icon: 'shield',
          title: 'Танҳо маҳсулоти боэътимод',
          description: 'Маҳсулоти аслӣ аз брендҳои беҳтарин',
        },
        {
          id: 'care',
          icon: 'heart',
          title: 'Ғамхорӣ ба шумо',
          description: 'Мо дар ҳар марҳилаи нигоҳубин ҳамроҳи шумоем',
        },
        {
          id: 'delivery',
          icon: 'truck',
          title: 'Расонидани осон',
          description: 'Тез дар Душанбе ва тамоми кишвар',
        },
      ],
    },
    categories: {
      heading: 'Категорияҳои маъмул',
      viewAll: 'Ҳамаро дидан',
      items: [
        {
          id: 'face',
          title: 'Нигоҳубини рӯй',
          subtitle: 'Ғамхорӣ ба пӯсти шумо',
          featured: true,
          image: 'https://hbshop.tj/media/items/Frame_2_-_2025-04-01T1707.jpg',
        },
        { id: 'hair', title: 'Нигоҳубини мӯй', image: 'https://hbshop.tj/media/items/Frame_2_Lhnhm7i.jpg' },
        { id: 'makeup', title: 'Ороиш', image: 'https://hbshop.tj/media/items/201_eWDP7ti.jpg' },
        { id: 'body', title: 'Бадан', image: 'https://hbshop.tj/media/items/Frame_2_-_2025-02-22T1445.jpg' },
        { id: 'fragrance', title: 'Атрҳо', image: 'https://hbshop.tj/media/items/Frame_1_3_PihwfCP.jpg' },
      ],
    },
    catalogPage: {
      heading: 'Феҳрист',
      subheading: 'Беҳтарин маҳсулот дар ҳар категория',
      loading: 'Маҳсулот бор карда мешавад...',
      error: 'Феҳрист бор нашуд. Лутфан саҳифаро аз нав бор кунед.',
      empty: 'Дар ин категория ҳоло маҳсулот нест.',
    },
    ai: {
      brand: 'HAYAT BEAUTY',
      role: 'AI Beauty Advisor',
      greeting: 'Салом! Ман мушовири AI-и HAYAT BEAUTY ҳастам. Имрӯз чӣ тавр кӯмак карда метавонам?',
      welcomeTitle: 'HAYAT BEAUTY AI',
      welcomeSubtitle: 'Мушовири шахсии зебоии шумо',
      welcomeDescription: 'Ба шумо дар интихоби нигоҳубин, косметика ва маҳсулот аз феҳристи HAYAT BEAUTY кӯмак мерасонам.',
      quickActions: [
        { id: 'skincare', label: 'Нигоҳубини пӯст', prompt: 'Нигоҳубини пӯстро барои ман маслиҳат диҳед' },
        { id: 'makeup', label: 'Ороиш', prompt: 'Ба ман дар интихоби ороиш кӯмак кунед' },
        { id: 'hair', label: 'Мӯй', prompt: 'Ба нигоҳубини мӯй ниёз дорам' },
        { id: 'fragrance', label: 'Атрҳо', prompt: 'Атреро маслиҳат диҳед' },
      ],
      recommendationsLabel: 'Тавсияҳо барои шумо',
      placeholder: 'Нависед, ки чӣ шуморо ҷалб мекунад...',
      send: 'Фиристодан',
      typing: 'AI менависад...',
      addToCart: 'Ба сабад',
      added: 'Илова шуд',
      cheaperOption: 'Арзонтар?',
      cheaperUserLine: 'Ин каме қиммат аст',
      cheaperFound: 'Албатта, инак варианти арзонтар бо таъиноти шабеҳ.',
      cheaperUnavailable: 'Ин аллакай арзонтарин вариант дар ин категория аст.',
      closeChat: 'Пӯшидани чат',
      closeBubble: 'Пинҳон кардан',
      error: 'Мебахшед, хатое рух дод. Лутфан бори дигар кӯшиш кунед.',
      purchaseMessage:
        'Фармоиши шумо #{orderId} бо муваффақият расмият дода шуд. Ман дастури истифодаи воситаҳои интихобшударо омода кардам. Забони дастурро интихоб кунед:',
      languageRu: 'Русский',
      languageTj: 'Тоҷикӣ',
      pdfTitle: 'Дастури истифодаи воситаҳои HAYAT BEAUTY',
      pdfCaptionRu: 'Русский · PDF',
      pdfCaptionTj: 'Тоҷикӣ · PDF',
      pdfDownload: 'Боргирии дастур',
      pdfPreparing: 'PDF омода карда мешавад...',
      pdfError: 'PDF омода нашуд. Лутфан бори дигар кӯшиш кунед.',
      pdfRetry: 'Такрор',
    },
    cart: {
      title: 'Сабад',
      empty: 'Сабад холӣ аст',
      total: 'Ҳамагӣ',
      currency: 'сомонӣ',
      checkout: 'Расмият додани фармоиш',
      checkingOut: 'Дар ҳоли расмиятдиҳӣ...',
      remove: 'Хориҷ кардан',
      checkoutReviewTitle: 'Расмият додани фармоиш',
      confirmOrder: 'Тасдиқи фармоиш',
      confirmingOrder: 'Фармоиши шумо расмият дода мешавад...',
      backToCart: 'Бозгашт',
      demoDisclaimer: 'Фармоиши намунавӣ — пардохт анҷом дода намешавад.',
      successTitle: 'Фармоиш бо муваффақият расмият дода шуд',
      successText: 'Ташаккур барои харид аз HAYAT BEAUTY.',
      instructionsHint: 'Дастур оид ба маҳсулот дар мушовири AI пайдо мешавад.',
      continueShopping: 'Идомаи харид',
      close: 'Пӯшидани сабад',
    },
    footer: {
      about: 'HAYAT BEAUTY — косметикаи премиум дар Душанбе бо мушовири шахсии зебоии AI.',
      contactsHeading: 'Тамос',
      address: 'ш. Душанбе, хиёбони Рӯдакӣ',
      phone: '+992 000 00 00 00',
      rights: 'Ҳама ҳуқуқҳо ҳифз шудаанд.',
    },
    auth: {
      loginTab: 'Вуруд',
      registerTab: 'Бақайдгирӣ',
      nameLabel: 'Ном',
      namePlaceholder: 'Номи шумо',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      passwordLabel: 'Парол',
      passwordPlaceholder: '••••••••',
      loginSubmit: 'Ворид шудан',
      registerSubmit: 'Сохтани аккаунт',
      loginPending: 'Вуруд...',
      registerPending: 'Сохтани аккаунт...',
      close: 'Пӯшидан',
      loggedInAs: 'Шумо ворид шудед ҳамчун',
      logout: 'Баромадан',
      account: 'Ҳисоби шахсӣ',
    },
    account: {
      title: 'Ҳисоби шахсӣ',
      purchaseHistory: 'Таърихи харидҳо',
      empty: 'То ҳол харид нест',
      quantity: 'Миқдор',
      purchasedOn: 'Харидашуда',
      depletionLabel: 'Тахминан кофӣ аст',
      loading: 'Боргирӣ...',
      loadError: 'Таърихи харидҳоро бор карда натавонист',
      notifications: 'Огоҳиномаҳо',
      notificationsEmpty: 'То ҳол огоҳинома нест',
      simulate24h: 'Шабеҳсозӣ: 24 соат гузашт',
      cartReminder: 'Фаромӯш накунед фармоишро расмият диҳед — дар сабад маҳсулот боқӣ мондааст',
      depletionWarning: '{title} ба зудӣ тамом мешавад',
    },
  },
}

export default translations
